...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($mode == 'vendors') {
        ...
        return array(CONTROLLER_STATUS_OK, fn_url('products.vendors?product_id=' . $_REQUEST['product_id']));
...
if ($mode == 'manage' || $mode == 'p_subscr') {
    ...
    $valid_page = db_get_valid_page($page, $search['items_per_page'], $search['total_items']);
 	$vendor_sell_data = db_get_array( 'SELECT * FROM ?:product_vendor_sell WHERE company_id != 0');
	$vendor_sell_product = array();
	if($vendor_sell_data){
    foreach($vendor_sell_data as $v) {
        $vendor_sell_product[$v['product_id']][] = $v;
    }
	}
	if ($page > $valid_page) {
	 ...
	 Tygh::$app['view']->assign('vendor_sell_product', $vendor_sell_product);
...
if($mode == 'vendors'){
    ...
    $redirect_url = fn_url('products.vendors?product_id=' . $_REQUEST['product_id']);
