...
if ($mode == 'print_invoice') {
    ...
    unset($all_params['status']);
    list($orders_for_statuses, $all_search) = fn_get_orders($all_params);
    $order_counts_for_statuses = array();
    foreach($orders_for_statuses as $orderKey => $orderDetails){
        $orders_for_status_info = fn_get_order_info($orderDetails['order_id']);
        if(!isset($order_counts_for_statuses[$orders_for_status_info['status']])){
            $order_counts_for_statuses[$orders_for_status_info['status']]['count'] = 0;
            $order_counts_for_statuses[$orders_for_status_info['status']]['total'] = 0;
        }
        $order_counts_for_statuses[$orders_for_status_info['status']]['status_name'] = $orders_for_status_info['status'];
        $order_counts_for_statuses[$orders_for_status_info['status']]['status'] = $orders_for_status_info['status'];
        $order_counts_for_statuses[$orders_for_status_info['status']]['count']++;
        $order_counts_for_statuses[$orders_for_status_info['status']]['total'] += $orders_for_status_info['total'];
        $order_counts_for_statuses[$orders_for_status_info['status']]['shipping'] = $orders_for_status_info['shipping_cost'];
    }

    foreach($order_by_statuses as $order_by_status_key => $order_by_status_details) {
        if(isset($order_counts_for_statuses[$order_by_status_details['status']])){
            $order_by_statuses[$order_by_status_key]['count'] = $order_counts_for_statuses[$order_by_status_details['status']]['count'];
            $order_by_statuses[$order_by_status_key]['total'] = $order_counts_for_statuses[$order_by_status_details['status']]['total'];
            $order_by_statuses[$order_by_status_key]['shipping'] = $order_counts_for_statuses[$order_by_status_details['status']]['shipping'];
        } else {
            $order_by_statuses[$order_by_status_key]['count'] = 0;
            $order_by_statuses[$order_by_status_key]['total'] = 0.00;
            $order_by_statuses[$order_by_status_key]['shipping'] = 0.00;
        }
    }
...
Tygh::$app['view']->assign('order_by_statuses', $order_by_statuses);
    ...
    list($orders, $search) = fn_get_orders($all_params);

    foreach($orders as $orderKey => $orderDetails){
        $orders[$orderKey]['order_details'] = fn_get_order_info($orderDetails['order_id']);
        $orders[$orderKey]['total_tax'] = 0;
        foreach($orders[$orderKey]['order_details']['products'] as $orderDetailKey => $orderDetail){
            $orders[$orderKey]['total_tax'] += $orderDetail['tax_value'];
        }
    }
    $totals = array();
    $totals['all_order_amount'] = array_sum(array_column($orders, 'total'));
    $totals['all_tax_amount'] = array_sum(array_column($orders, 'total_tax'));
    ...
    $totals['order_amount'] = array_sum(array_column($orders, 'total'));
    $totals['tax_amount'] = array_sum(array_column($orders, 'total_tax'));
