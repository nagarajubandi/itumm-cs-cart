...
// Check password content
                if (Registry::get('settings.Security.admin_passwords_must_contain_mix') == 'Y') {
                    $tmp_result = preg_match('/\d+/',$_REQUEST['company_data']['vendor_temp']) && preg_match('/\D+/', $_REQUEST['company_data']['vendor_temp']);
                    if (!$tmp_result) {
...
/*
 ...
 $mailer->send(array(
                    ...
                    'from' => 'rajesh.ggt@gmail.com', 
                    'data' => array(),
                    'tpl' => 'companies/welcome_email.tpl', 
                ), 'A', Registry::get('settings.Appearance.backend_default_language'));
