    ...
    if ($mode == 'update') {
        ...
        if ($res) {
            ...
            if ($action === 'get_auth_token') {
                list($auth_token) = fn_get_user_auth_token($user_id);
            }
            ...
            if (!empty($_REQUEST['return_url'])) {
                ...
                if ($action === 'get_auth_token' && isset($auth_token)) {
                    $return_url = fn_link_attach($return_url, 'token=' . $auth_token);
                }
        ...
        return array(CONTROLLER_STATUS_OK, Url::buildUrn($redirect_dispatch, $redirect_params));
    }
}
