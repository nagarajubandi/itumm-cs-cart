...
class VendorPayouts
{
    ...
    public function getList(array $params, $items_per_page = 0)
    {
        ...
        $join = $this->db->quote(
            ' LEFT JOIN ?:orders AS orders ON (payouts.order_id = orders.order_id AND ((payouts.payout_type = "order_placed") AND (orders.status = "C")))'
        ...
        $fields = array(
            ...
            'payment_method' => "IF(payouts.order_id <> 0, IF(orders.payment_id = 6, 'COD', 'EBS'), '')",
            'payout_amount' => 'IF(payouts.order_id <> 0, payouts.order_amount, payouts.payout_amount)',
            'commission_amount' => 'IF(payouts.order_id <> 0 AND orders.payment_id = 6, 0, payouts.commission_amount)',
            'commission' => 'IF(payouts.order_id <> 0 AND orders.payment_id = 6, 0, payouts.commission)',
    ...
    public function getIncome($params = array())
    {
        ...
        $join = $this->db->quote('LEFT JOIN ?:orders AS orders ON (payouts.order_id = orders.order_id AND ((payouts.payout_type = "order_placed") AND (orders.status = "C")))');
