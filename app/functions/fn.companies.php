    ...
    if (empty($company_id)) {
        ...
        if (fn_allowed_for('ULTIMATE')) {
            ...
            $company_placement_info = array(
                ...
                'company_gst_reg_no' => $company['gst_reg_no'],
...
function fn_create_company_admin($company_data, $fields = '', $notify = false)
{
	...
	if(isset($company_data['vendor_temp']) && !empty($company_data['vendor_temp']))
	{
		$user['password1'] = $company_data['vendor_temp'];
		 $user['password_change_timestamp'] = time();
