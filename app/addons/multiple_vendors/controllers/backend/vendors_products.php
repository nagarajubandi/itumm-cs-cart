...
if (!Registry::get('runtime.company_id')) {
	...
	if ($_SERVER['REQUEST_METHOD'] == 'POST') {
		...
		if ($mode == 'm_update') {
			...
			return array(CONTROLLER_STATUS_OK, "vendors_products.manage");
		}
		...
		if ($mode == 'update') {
			...
			if (!empty($_REQUEST['product_data'])) {
				...
				if (!empty($isset_product)) {
                    ...
                    if (!empty($_REQUEST['product_data']['amount'])) {
                        db_query ("UPDATE ?:vendors_products_data SET amount = ?i WHERE id = ?i", $_REQUEST['product_data']['amount'], $isset_product);
                    }
				} else {
			...
			return array(CONTROLLER_STATUS_OK, "vendors_products.update?product_id=".$_REQUEST['product_id']);
		}
	...
	if ($mode == 'manage') {
	    ...
	    if (!empty($action) && $action == 'last_view') {
	        return array(CONTROLLER_STATUS_REDIRECT, 'vendors_products.manage');
        }
		...
		fn_add_breadcrumb(fn_get_lang_var('products'), "vendors_products.manage.reset_view");

		fn_add_breadcrumb(fn_get_lang_var('search_results'), "vendors_products.manage.last_view");
