...
if ($mode == 'list') {
    ...
    Registry::get('view')->assign(array(
        'payouts' => $payouts,
        'search' => $search,
        'payout_search_data' => $payout_search_data
    ));
...
function fn_get_customer_payouts($params, $items_per_page = 0, $auth, $lang_code = CART_LANGUAGE)
{
    ...
    $payouts = db_get_hash_array('SELECT ?:affiliate_payouts.*, ?:users.firstname, ?:users.lastname FROM ?:affiliate_payouts LEFT JOIN ?:users ON ?:affiliate_payouts.partner_id=?:users.user_id WHERE ?p AND ?:users.user_id = ?i ?p ?p', 'payout_id', $payout_search_condition, $auth['user_id'], $sorting, $limit);
