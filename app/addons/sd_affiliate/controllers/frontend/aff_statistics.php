...
if ($mode == 'commissions') {
    ...
    if (!empty($_REQUEST['period']) && $_REQUEST['period'] != 'A') {
        ...
        $statistic_conditions .= db_quote(' AND (actions.date >= ?i AND actions.date <= ?i)', $_REQUEST['time_from'], $_REQUEST['time_to']);
    ...
    if (!empty($statistic_search_data['status'])) {
        ...
        foreach ($statistic_search_data['status'] as $_status) {
            ...
            if ($_status == CommissionStatuses::PAIDUP) {
                $_conditions .= ' (actions.payout_id != 0) ';
            } elseif ($_status == CommissionStatuses::APPROVED) {
