...
if (!empty($auth['user_id'])) {
    ...
    if (!empty($partner_data)) {
        ...
        Registry::get('view')->assign(array(
            'total_commissions' => db_get_field("SELECT SUM(amount) FROM ?:aff_partner_actions WHERE partner_id = ?i AND approved = 'Y'", $auth['user_id']),
            'max_amount' => $max_amount,
            'last_payouts' => $last_payouts,
            'partner' => $partner_data,
            'partners' => $partners,
            'affiliate_plans' => fn_get_affiliate_plans_list()
        ));
    }
}
/** /Body **/
