...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($mode == 'update') {
        if (empty($_REQUEST['group']['data'])) {
            fn_set_notification('W', __('warning'), __('addons.sd_affiliate.product_group_empty_data'));
        }
...
function fn_get_groups($params, $items_per_page = 0, $lang_code = DESCR_SL)
{
    ...
    $default_params = array (
        'link_to' => BannerLinkTypes::CATEGORIES,
        'page' => 1,
