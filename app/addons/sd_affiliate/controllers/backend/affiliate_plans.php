...
function fn_get_affiliate_plans($params, $items_per_page = 0, $lang_code = CART_LANGUAGE)
{
    ...
    $plans = db_get_hash_array(
        "SELECT plans.plan_id, plans.company_id, object as name, COUNT(user_id) as count_partners, status "
        . "FROM ?:affiliate_plans as plans "
        . "LEFT JOIN ?:common_descriptions as com_descr ON object_holder = 'affiliate_plans' AND object_id = plans.plan_id AND lang_code = ?s "
        . "LEFT JOIN ?:aff_partner_profiles ON plans.plan_id = ?:aff_partner_profiles.plan_id AND ?:aff_partner_profiles.approved = ?s "
        . "WHERE 1 GROUP BY plans.plan_id ORDER BY object $limit ", 'plan_id', $lang_code, UserStatuses::APPROVED);
