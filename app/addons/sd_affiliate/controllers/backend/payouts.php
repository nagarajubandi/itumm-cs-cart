    ...
    if ($mode == 'do_m_pay_affiliates') {
        if (!empty($_REQUEST['partner_ids'])) {
            ...
            foreach ($_REQUEST['partner_ids'] as $_partner_id) {
                $_where_condition = db_quote("approved = ?s AND payout_id = ?i AND partner_id = ?s $_date_condition", 'Y', '0', $_partner_id);
                    ...
                    if (!empty($user_id)) {
                        ...
                        if (!empty($amount)) {
                            if (fn_update_partner_balance($user_id, $amount, '-')) {
                                $payout_id = db_query('INSERT INTO ?:affiliate_payouts ?e', array('partner_id' => $user_id, 'amount' => $amount, 'status' => AffiliateStatuses::OPEN, 'date' => TIME));
                                if (!empty($payout_id)) {
...
if ($mode == 'manage') {
    ...
    list($partner_list) = fn_get_users(array('user_type' => UserTypes::PARTNER, 'active' => 'Y', 'approved' => UserStatuses::APPROVED), $auth);

    $_partner_list = array();
    foreach ($partner_list as $partner_data) {
        if (!empty($partner_data['firstname']) || !empty($partner_data['lastname'])) {
            $_partner_list[$partner_data['user_id']] = $partner_data['firstname'] . ' ' . $partner_data['lastname'] . ' (' . __("user_id") . ': ' . $partner_data['user_id'] . ') ';
        }else {
            $_partner_list[$partner_data['user_id']] = __("affiliate") . '_' . $partner_data['user_id'];
        }
    }
    ...
    $payment_period_options = Settings::instance()->getVariants('sd_affiliate', 'payment_period');
    Registry::get('view')->assign('period_name', $payment_period_options[Registry::get('addons.sd_affiliate.payment_period')]);
    ...
    if (!empty($payout_data['partner_id'])) {
        ...
        $payout_data['partner']['user_type'] = 'P';
        $payout_data['partner']['partner_exists'] = fn_sd_affiliate_user_exists($payout_data['partner_id']);
        if (!empty($payout_data['partner']['firstname']) || !empty($payout_data['partner']['lastname'])) {
            $payout_data['partner']['affiliate'] = $payout_data['partner']['firstname'] . ' ' . $payout_data['partner']['lastname'];
        } else {
            $payout_data['partner']['affiliate'] = __("affiliate") . '_' . $payout_data['partner_id'];
        }
    }
    ...
    Registry::get('view')->assign(array(
        'payouts' => array($payout_data['partner_id'] => $payout_data),
        'search' => $search
    ));
...
function fn_get_payouts($params, $items_per_page = 0)
{
    ...
    // Define sort fields
    $sortings = array (
        'id' => "?:affiliate_payouts.payout_id",
        'email' => '?:users.email',
    ...
    if (!empty($params['partner_id'])) {
        $condition .= db_quote(' AND ?:affiliate_payouts.partner_id = ?i', $params['partner_id']);
    }
    ...
    if (!empty($params['items_per_page'])) {
        $params['total_items'] = db_get_field('SELECT COUNT(*) FROM ?:affiliate_payouts LEFT JOIN ?:users ON ?:affiliate_payouts.partner_id = ?:users.user_id WHERE 1 ?p', $condition);
        $limit = db_paginate($params['page'], $params['items_per_page']);
    }

    $payouts = db_get_hash_array('SELECT ' . implode(', ', $fields) . ' FROM ?:affiliate_payouts LEFT JOIN ?:users ON ?:affiliate_payouts.partner_id = ?:users.user_id WHERE 1 ?p ?p ?p', 'payout_id', $condition, $sorting, $limit);

    foreach ($payouts as $payout_id => $payout) {
        $payouts[$payout_id]['user_type'] = 'P';
        $payouts[$payout_id]['partner_exists'] = fn_sd_affiliate_user_exists($payout['partner_id']);
        if (!empty($payout['firstname']) || !empty($payout['lastname'])) {
            $payouts[$payout_id]['affiliate'] = $payout['firstname'] . ' ' . $payout['lastname'];
        } else {
            $payouts[$payout_id]['affiliate'] = __("affiliate") . '_' . $payout['partner_id'];
        }
    }
...
function fn_pay_affiliates($params, $items_per_page = 0)
{
    ...
    $sorting = db_sort($params, $sortings, 'user_id', 'asc');
    ...
    $partner_balances = db_get_hash_array('SELECT pa.partner_id, u.firstname, u.lastname, u.email, SUM(amount) as amount FROM ?:aff_partner_actions as pa LEFT JOIN ?:users as u ON pa.partner_id = u.user_id LEFT JOIN ?:aff_partner_profiles as pp ON pa.partner_id = pp.user_id LEFT JOIN ?:affiliate_plans as ap ON ap.plan_id = pp.plan_id WHERE pa.approved = ?s AND payout_id = ?i ?p ?p ?p ?p', 'partner_id', 'Y', 0, $condition, $group, $sorting, $limit);

    foreach ($partner_balances as $balance_id => $balance) {
        $partner_balances[$balance_id]['user_type'] = 'P';
        $partner_balances[$balance_id]['partner_exists'] = fn_sd_affiliate_user_exists($balance['partner_id']);
        if (!empty($balance['firstname']) || !empty($balance['lastname'])) {
            $partner_balances[$balance_id]['affiliate'] = $balance['firstname'] . ' ' . $balance['lastname'];
        } else {
            $partner_balances[$balance_id]['affiliate'] = __("affiliate") . '_' . $balance['partner_id'];
        }
    }
    ...
    if (!empty($params['partner_ids'])) {
        ...
        foreach ($params['partner_ids'] as $_partner_id) {
            ...
            if (empty($new_payouts[$_partner_id]['amount'])) {
                ...
                $partner_action_fields = array('approved', 'payout_id', 'partner_id', 'date');
                foreach($partner_action_fields as $field_name) {
                    $params['condition'] = str_replace(' ' . $field_name, ' actions.' . $field_name, $params['condition']);
                }
...
function fn_get_previous_payouts($params, $items_per_page = 0, $lang_code = DESCR_SL)
{
    ...
    $previous_payouts = db_get_hash_array('SELECT aff_pay.partner_id, AVG(aff_pay.amount) as avg_amount, SUM(aff_pay.amount) as total_amount, users.firstname, users.lastname, users.email, p_profiles.balance FROM ?:affiliate_payouts as aff_pay LEFT JOIN ?:users as users ON aff_pay.partner_id = users.user_id LEFT JOIN ?:aff_partner_profiles as p_profiles ON aff_pay.partner_id = p_profiles.user_id GROUP BY aff_pay.partner_id ?p ?p', 'partner_id', $sorting, $limit);
    ...
    if (!empty($previous_payouts)) {
        ...
        foreach ($previous_payouts as $user_id => $prev_payout) {
            $previous_payouts[$user_id]['user_type'] = 'P';
            $previous_payouts[$user_id]['partner_exists'] = fn_sd_affiliate_user_exists($prev_payout['partner_id']);
            ...
            if (!empty($prev_payout['firstname']) || !empty($prev_payout['lastname'])) {
                $previous_payouts[$user_id]['affiliate'] = $prev_payout['firstname'] . ' ' . $prev_payout['lastname'];
            } else {
                $previous_payouts[$user_id]['affiliate'] = __("affiliate") . '_' . $user_id;
            }
        }
    }
