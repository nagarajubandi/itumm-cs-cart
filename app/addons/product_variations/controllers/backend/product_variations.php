...
if ($mode == 'generate') {
    ...
    if (count($selected_options_ids) > 0) {
        // fn_get_options_combinations returns the options in reverse order
        $selected_options_ids = array_reverse($selected_options_ids, true);
...
function fn_product_variations_generate($product_id, $combinations, array $options_ids)
{
    ...
    $tracking = fn_product_variations_normalize_tracking($product_row['tracking']);

    db_query(
        'UPDATE ?:products SET product_type = ?s, variation_options = ?s, tracking = ?s WHERE product_id = ?i',
        ProductManager::PRODUCT_TYPE_CONFIGURABLE, json_encode(array_values($options_ids)), $tracking, $product_id
    );
}
