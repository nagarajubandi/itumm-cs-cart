    ...
    <td valign="top">
        ...
        {if $settings.General.quick_registration == "Y"}
            <tr>
                ...
                <td>{if $user_data.company_id}{"auth.login_form&company_id=`$user_data.company_id`"|fn_url:"C"}{else}{"auth.login_form"|fn_url:"C"}{/if}</td>
            </tr>
        {/if}
...
{if $user_data.user_type == 'P' && count($_REQUEST.user_data) == 4}
  {$profile_fields.C = ''}
{/if}
