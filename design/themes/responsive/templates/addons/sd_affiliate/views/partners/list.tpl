        ...
        <div class="ty-affiliate-table-wrapper">
            ...
            <table class="ty-affiliate-summary__table">
                {foreach from=$last_payouts item=period name=last_payout}
                    {if $period.range.start >= $user_data_register && $period.range.start <= {html_select_date|time} && !empty($period.amount)}
                        <tr class="ty-affiliate-summary__row">
                            <td {if $smarty.foreach.last_payout.first}class="no-border"{/if}>
                                {if $period.amount > 0}
                                    <a href="{"aff_statistics.commissions?action=search&statistic_search[partner_id]=`$partner.user_id`&statistic_search[status][A]=A&statistic_search[status][P]=P&statistic_search[period]=C&time_from=`$period.range.start`&time_to=`$period.range.end`"|fn_url}">{$period.range.start|date_format:$settings.Appearance.date_format}</a>
                                {/if}
                            </td>
