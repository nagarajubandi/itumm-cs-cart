{if $product.discussion_type && $product.discussion_type != 'D'}
    <div class="ty-discussion__rating-wrapper clearfix" id="average_rating_product_{$obj_prefix}{$obj_id}">
        ...
        {/if}
        <a style="display:none;" class="ty-discussion__review-write cm-dialog-opener cm-dialog-auto-size" data-ca-target-id="new_post_dialog_{$obj_prefix}{$obj_id}" rel="nofollow">{__("write_review")}</a>
