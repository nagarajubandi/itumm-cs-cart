...
{assign var="videos" value=$product.product_id|fn_cc_youtube_get_product_videos}
...
{if $videos}
    ...
    {/if}
    <div class="video-gallery-tab ty-product-img cm-preview-wrapper" style="{if $videos|count != 1}display: none;{/if}">
        {assign var="main_video" value=$videos|array_rand:1}
        <iframe id="youtube_galary_tab" class="youtube-video" width="100%" src="https://www.youtube-nocookie.com/embed/{$videos.$main_video}?{if $addons.cc_youtube.show_suggested_videos_after_finish == 'Y'}rel=0&amp;{/if}controls={$show_player_controls}&amp;showinfo={$show_title_and_actions}" frameborder="0" allowfullscreen></iframe>
    </div>
            ...
            {if $videos|count > 1}
                {foreach from=$videos item="video_image" key="product_id"}
                    <div class="cm-item-gallery ty-float-left">
                        ...
                        <a data-ca-gallery-large-id="det_img_link_{$product_id}_{$video_image}" onclick='gallery_show_video("{$video_image}", {$show_player_controls}, "tab", {$show_title_and_actions})' class="cm-gallery-item" style="width: {$th_size}px">
