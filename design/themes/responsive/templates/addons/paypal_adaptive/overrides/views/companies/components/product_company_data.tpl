{if "MULTIVENDOR"|fn_allowed_for && ($company_name || $company_id) && $settings.Vendors.display_vendor == "Y"}
    <div class="ty-control-group{if !$capture_options_vs_qty} product-list-field{/if}  {if !empty($product.paypal_verification.verified) && $product.paypal_verification.verified == 'verified'}paypal-adaptive-vendor-name-text{/if}">
