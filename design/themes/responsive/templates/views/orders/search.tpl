...
{/capture}
{include file="common/section.tpl" section_title=__("search_options") section_content=$smarty.capture.section class="ty-search-form" collapse=false}
...
<table class="ty-table ty-orders-search">
    ...
    {foreach from=$orders item="o"}
        <tr>
            ...
            <td class="ty-orders-search__item">{$o.timestamp|date_format:"`$settings.Appearance.date_format`, `$settings.Appearance.time_format`"}</td>

            {hook name="orders:manage_data"}{/hook}

            <td class="ty-orders-search__item">{include file="common/price.tpl" value=$o.total}</td>
            <td class="ty-orders-search__item"><a class="ty-btn__primary ty-btn" href="{"orders.details?order_id=`$o.order_id`"|fn_url}">View / Modify Order</a></td>
        </tr>
    {foreachelse}
