    ...
    <ul class="ty-account-info">
        {hook name="profiles:my_account_menu"}
            {capture name="title"}
				...
				<div class="my_account_sign_in">
{if $auth.user_id}
                <li>Hello {$user_info.firstname}</li>
                {else}
 <a href="{if $runtime.controller == "auth" && $runtime.mode == "login_form"}{$config.current_url|fn_url}{else}{"auth.login_form?return_url=`$return_current_url`"|fn_url}{/if}" {if $settings.Security.secure_storefront != "partial"} data-ca-target-id="login_block{$block.snapping_id}" class="cm-dialog-opener cm-dialog-auto-size "{else} class="ty-btn ty-btn__primary"{/if} rel="nofollow">{__("sign_in")}</a>
                {/if}
