...
{hook name="products:product_name"}
    {if $show_name}
        {if $hide_links}<strong>{else}<a href="{"products.view?product_id=`$product.product_id`"|fn_url}" class="product-title" title="{$product.product|strip_tags}" {live_edit name="product:product:{$product.product_id}" phrase=$product.product}>{/if}{$product.product|truncate:85:"...":true nofilter}{if $hide_links}</strong>{else}</a>{/if}
                ...
                {if (
                    ...
                    <div class="ty-control-group product-list-field">
                        ...
                        {elseif $product_amount > 0}
                            <span class="ty-qty-out-of-stock ty-control-group__item" id="in_stock_info_{$obj_prefix}{$obj_id}">
                                Only {$product_amount} left!
                            </span>
                        {else}
                            <span class="ty-qty-in-stock ty-control-group__item" id="in_stock_info_{$obj_prefix}{$obj_id}">
                                {__("on_backorder")}
