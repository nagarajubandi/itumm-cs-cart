                    ...
                    {if !$avail_sorting || $avail_sorting[$sort_key] == 'Y'}
                        <li class="{$sort_class} ty-sort-dropdown__content-item">
                            {hook name="ab__so_seohide:sorting_link"}
                                <a class="{$ajax_class} ty-sort-dropdown__content-item-a" data-ca-target-id="{$pagination_id}" href="{"`$curl`&sort_by=`$option`&sort_order=`$sort_order`&abcd__update=Y"|fn_url}" rel="nofollow">{__("sort_by_`$option`_`$sort_order`")}</a>
                            {/hook}
                        </li>
                    {/if}
                {/if}
