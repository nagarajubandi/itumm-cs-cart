...
<div class="ty-tabs cm-j-tabs{if $track} cm-track{/if} clearfix">
    ...
    {foreach from=$navigation.tabs item=tab key=key name=tabs}
        {if $navigation.ab__grid_tab == 'Y'}
            {if ((!$tabs_section && !$tab.section) || ($tabs_section == $tab.section)) && !$key|in_array:$empty_tab_ids && substr($key, 0, 12) == 'ab__grid_tab'}
            {if !$active_tab}
                {assign var="active_tab" value=$key}
            {/if}
            {assign var="_tabs" value=true}
            <li id="{$key}" {if $tab.ab__grid_tabs}data-url="{$tab.href|fn_url}"{/if} class="{if $tab.ab__grid_tabs}ab__grid_tabs {/if}ty-tabs__item{if $tab.js} cm-js{elseif $tab.ajax} cm-js cm-ajax{/if}{if $key == $active_tab} active{/if}"><a class="ty-tabs__a" {if !$tab.ab__grid_tabs and $tab.href} href="{$tab.href|fn_url}"{/if}>{$tab.title}</a></li>
            {/if}
        {else}
