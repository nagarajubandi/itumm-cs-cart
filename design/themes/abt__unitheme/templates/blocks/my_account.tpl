{** block-description:my_account ty-account-infoty-account-infoty-account-infoty-account-info**}

{capture name="title"}
    <a class="ty-account-info__title" href="{"profiles.update"|fn_url}">
        <i class="ty-icon-user"></i>&nbsp;
        <span class="ty-account-info__title-txt" {live_edit name="block:name:{$block.block_id}"}><small>{__("your_personal")}</small>{$title}</span>
        <i class="ty-icon-down-micro ty-account-info__user-arrow"></i>
		{if $auth.user_id}
              <br/>  {if $user_info.firstname || $user_info.lastname}
                    <span class="ty-account-info__item  ty-account-info__name ty-dropdown-box__item" style="color: #ffffff"><strong> <span title="{$user_info.firstname} {$user_info.lastname}">Hello {$user_info.firstname|substr:0:12}...!</span> </strong></span>
		
                    <span class="ty-account-info__item ty-dropdown-box__item">&nbsp;</span>
                {else}
                    <span class="ty-account-info__item ty-dropdown-box__item ty-account-info__name" style="color: #ffffff">{$user_info.email}</span>
                {/if}
		 {/if}
    </a>
{/capture}
