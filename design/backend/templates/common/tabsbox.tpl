...
{capture name="tab_items"}
    ...
    {foreach from=$navigation.tabs item=tab key=key name=tabs}
        ...
        {if ($runtime.company_id)}
         {if $key == "views"}  		
		<li style="display:none" id="{$key}{$id_suffix}" class="{if $tab.hidden == "Y"}hidden {/if}{if $tab.js}cm-js{elseif $tab.ajax}cm-js cm-ajax{if $tab.ajax_onclick} cm-ajax-onclick{/if}{/if}{if $key == $active_tab} active{/if} {if $tab.properties}extra-tab{/if}">
