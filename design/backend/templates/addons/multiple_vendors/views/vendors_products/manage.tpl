...
<tr class="cm-row-status-{$product.status|lower} {$hide_inputs_if_shared_product}">
    {hook name="vendors_products:manage_body"}
    <td class="left">
    ...
    {/if}
    <td>
        {include file="common/image.tpl" image=$product.main_pair.icon|default:$product.main_pair.detailed image_id=$product.main_pair.image_id image_width=$settings.Thumbnails.product_admin_mini_icon_width image_height=$settings.Thumbnails.product_admin_mini_icon_height href="vendors_products.update?product_id=`$product.product_id`"|fn_url}
    </td>
    <td>
        ...
        <a class="row-status" title="{$product.product|strip_tags}" href="{"vendors_products.update?product_id=`$product.product_id`"|fn_url}">{$product.product|truncate:40 nofilter}</a>
...
<div class="buttons-container">
    {include file="buttons/save_cancel.tpl" but_text=__("modify_selected") but_name="dispatch[vendors_products.store_selection]" cancel_action="close"}
</div>
{/capture}
...
{capture name="buttons"}
    ...
    {/capture}
    {dropdown content=$smarty.capture.tools_list}
     {include file="buttons/save.tpl" but_name="dispatch[vendors_products.m_update]" but_role="submit-button" but_target_form="manage_products_form"}
{/capture}
...
{capture name="sidebar"}
    {include file="common/saved_search.tpl" dispatch="vendors_products.manage" view_type="products"}
