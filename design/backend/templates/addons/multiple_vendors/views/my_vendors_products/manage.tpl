...
<tr class="cm-row-status-{$product.status|lower} {$hide_inputs_if_shared_product}">
    {hook name="my_vendors_products:manage_body"}
    <td class="left">
    ...
    {/if}
    <td>
        {include file="common/image.tpl" image=$product.main_pair.icon|default:$product.main_pair.detailed image_id=$product.main_pair.image_id image_width=$settings.Thumbnails.product_admin_mini_icon_width image_height=$settings.Thumbnails.product_admin_mini_icon_height href="my_vendors_products.update?product_id=`$product.product_id`"|fn_url}
    </td>
    <td>
        ...
        <a class="row-status" title="{$product.product|strip_tags}" href="{"my_vendors_products.update?product_id=`$product.product_id`"|fn_url}">{$product.product|truncate:40 nofilter}</a>
        <div class="product-code">
            <span class="product-code-label row-status">{__("sku")} </span>
            <input type="text" name="products_data[{$product.product_id}][product_code]" size="15" maxlength="32" value="{$product.product_code}" class="input-hidden span2" disabled="disabled"/>
        </div>
    ...
    <td>
        ...
        {*{include file="buttons/button.tpl" but_text=__("edit") but_href="product_options.inventory?product_id=`$product.product_id`" but_role="edit"}*}
            ...
            {include file="common/popupbox.tpl" id="edit_variants_inventory_`$product.product_id`" text="`$ttt`: `$product.product`" content=$smarty.capture.edit_variants_inventory link_text=__("edit") act="general"}
    ...
    {include file="common/select_popup.tpl" popup_additional_class="dropleft" id=$product.product_id status=$product.status hidden=true object_id_name="product_id" table="products" non_editable=true}
    </td>
</tr>
{/foreach}
</table>
...
<div class="buttons-container">
    {include file="buttons/save_cancel.tpl" but_text=__("modify_selected") but_name="dispatch[my_vendors_products.store_selection]" cancel_action="close"}
</div>
{/capture}
...
{capture name="buttons"}
    ...
    {/capture}
    {dropdown content=$smarty.capture.tools_list}
     {include file="buttons/save.tpl" but_name="dispatch[my_vendors_products.m_update]" but_role="submit-button" but_target_form="manage_products_form"}
{/capture}
...
{capture name="sidebar"}
    {include file="common/saved_search.tpl" dispatch="my_vendors_products.manage" view_type="products"}
    {include file="views/products/components/products_search_form.tpl" dispatch="my_vendors_products.manage"}
{/capture}
