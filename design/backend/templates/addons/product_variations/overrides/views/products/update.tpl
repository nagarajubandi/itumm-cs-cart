{capture name="mainbox"}
    ...
    {capture name="tabsbox"}
        ...
        {if $smarty.const.ACCOUNT_TYPE == "admin" && $product_data.product_id }
            {assign var="hide_input_for_admin" value=0}
        {else}
            {assign var="hide_input_for_admin" value=1}
        {/if}
            ...
            <div class="product-manage" id="content_detailed"> {* content detailed *}
                ...
                <div id="acc_information" class="collapse in">
                    ...
                    {if $product_type->isFieldAvailable("prices")}
                        {if $hide_input_for_admin}
                            ...
                            {/hook}
                        {else}
                            <input type="hidden" name="product_data[price]" id="elm_price_price" size="10" value="{$product_data.price|default:"0.00"|fn_format_price:$primary_currency:null:false}" class="input-long" />
                        {/if}
                    {/if}
                ...
                <div id="acc_pricing_inventory" class="collapse in">
                    ...
                    {if $hide_input_for_admin}
                        ...
                        {else}
                        <input type="hidden" name="product_data[amount]" id="elm_in_stock" size="10" value="{$product_data.amount|default:"1"}" class="input-small" />
                    {/if}
                    {/if}
                        ...
                        <div class="control-group" {if $product_data.product_type === "\Tygh\Addons\ProductVariations\Product\Manager::PRODUCT_TYPE_VARIATION"|constant}style="display:none;"{/if}>
                            ...
                            <div class="controls">
                                ...
                                {*{foreach from=$taxes item="tax"}
                                    <label class="radio inline" for="elm_taxes_{$tax.tax_id}">
                                        <input type="radio" name="product_data[tax_ids][]" id="elm_taxes_{$tax.tax_id}" {if $tax.tax_id|in_array:$product_data.tax_ids}checked="checked"{/if} value="{$tax.tax_id}" />
                                        {$tax.tax}</label>
                                    {foreachelse}
                                    &ndash;
                                {/foreach}*}
                                ...
                                <select class="span3" name="product_data[tax_ids][]" id="elm_taxes">
                                    ...
                                    {foreach from=$taxes item="tax"}
                                    <option value="{$tax.tax_id}" {if $tax.tax_id|in_array:$product_data.tax_ids}selected="selected"{/if}>{$tax.tax}</option>
                                    {foreachelse}
                                    {/foreach}
                                 </select>
                            </div>
