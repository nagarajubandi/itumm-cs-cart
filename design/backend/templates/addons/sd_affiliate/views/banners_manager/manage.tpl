...
{capture name="adv_buttons"}
    ...
    {if $link_type == $link_to}
        {include file="common/tools.tpl" tool_id="add_banner_{$link_type}" tool_href="banners_manager.add?banner_type=`$banner_type`&link_to=`$link_to`" prefix="top" hide_tools="true" title=__("affiliate_add_banner") icon="icon-plus"}
    {/if}
...
{include file="common/mainbox.tpl" title="{__("affiliate_banners")}: `$title`"  content=$smarty.capture.mainbox adv_buttons=$smarty.capture.adv_buttons buttons=$smarty.capture.buttons sidebar=$smarty.capture.sidebar}
