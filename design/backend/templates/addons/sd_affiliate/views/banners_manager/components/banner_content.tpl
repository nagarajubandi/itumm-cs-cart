...
{capture name="banner_content"}
    ...
    {elseif $banner_type == "js"}
        ...
        {else}
            {"aff_banner.view?bid=`$banner_data.banner_id`&type=js_content&sl=`$smarty.const.CART_LANGUAGE`&product_ids=`$banner_data.product_ids`&aff_id=$partner_id&force_new_window=Y"|fn_url nofilter}
        {/if}
    {/if}
{/capture}
{/strip}
