    ...
    <tr>
        ...
        <td>
            ...
            {elseif $row_stats.customer_id != 0}
                {if $row_stats.customer_exists}
                    <a href="{"profiles.update?user_id=`$row_stats.customer_id`"|fn_url}">{__('customer')}_{$row_stats.customer_id}</a>
                {else}
        ...
        <td colspan="9">
            ...
            {foreach from=$row_stats.extra_data item="r_action" name="related_action"}
                <tr>
                    ...
                    <td>{if $row_stats.partner_exists}<a href="{"profiles.update?user_id=`$r_action.partner_id`"|fn_url}">{$r_action.affiliate}</a>{else}{$r_action.affiliate}{/if}</td>
                <tr>
            {/foreach}
            </table>
            {/if}
