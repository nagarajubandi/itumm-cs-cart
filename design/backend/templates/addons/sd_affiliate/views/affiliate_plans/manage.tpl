    ...
    <form action="{""|fn_url}" method="post" name="manage_affiliate_plans_form">
        ...
        <table width="100%" class="table table-middle">
            ...
            {foreach from=$affiliate_plans item="aff_plan"}
                ...
                {if "ULTIMATE"|fn_allowed_for}
                    <tr class="cm-row-status-{$aff_plan.status|lower}{if !$allow_save || ($aff_plan.user_id == $smarty.session.auth.user_id)} cm-hide-inputs{/if}">
                {/if}
                    ...
                    <td class="center row-status">{$aff_plan.count_partners}</td>
