...
{/if}
    <td class="left"><input type="checkbox" name="user_ids[]" value="{$user.user_id}" class="cm-item"/></td>
    <td><a class="row-status" href="{"profiles.update?user_id=`$user.user_id`&user_type=`$user.user_type`"|fn_url}">{$user.user_id}</a></td>
    <td class="row-status">{if $user.firstname || $user.lastname}<a href="{"profiles.update?user_id=`$user.user_id`&user_type=`$user.user_type`"|fn_url}">{$user.lastname} {$user.firstname}</a>{else}-{/if}
    ...
    {else}
    <td width="10%" class="nowrap row-status">-</td>
    {/if}
    <td class="nowrap">
