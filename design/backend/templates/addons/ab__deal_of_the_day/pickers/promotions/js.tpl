...
{if $position_field}<td><input type="text" name="{$input_name}[{$promotion_id}]" value="{math equation="a*b" a=$position b=10}" size="3" class="input-micro"{if $clone} disabled="disabled"{/if} /></td>{/if}
...
<input class="cm-picker-value-description {$extra_class}" type="text" value="{$promotion}" {if $display_input_id}id="{$display_input_id}"{/if} size="10" name="page_name" readonly="readonly" {$extra}>&nbsp;
</span>
{/if}