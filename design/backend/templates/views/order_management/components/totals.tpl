<div class="pull-right order-notes statistic">
    <table>
        ...
        <span {if $cart.stored_taxes != "Y"}class="hidden"{/if} data-ca-switch-id="manual_taxes">
            ...
            <tr class="toggle-elm nowrap">
                <td class="right">&nbsp;CGST ({if $tax.price_includes_tax == "Y" && $settings.Appearance.cart_prices_w_taxes != "Y"}&nbsp;{__("included")}{/if}{strip}(<span {if $cart.stored_taxes == "Y"}class="hidden"{/if} data-ca-switch-id="manual_taxes">{include file="common/modifier.tpl" mod_value=$tax.rate_value/2 mod_type=$tax.rate_type})</span>

                </td>
                <td class="right">{include file="common/price.tpl" value=$tax.tax_subtotal/2}</td>
            </tr>
            <tr class="toggle-elm nowrap">
                <td class="right">&nbsp;SGST ({if $tax.price_includes_tax == "Y" && $settings.Appearance.cart_prices_w_taxes != "Y"}&nbsp;{__("included")}{/if}{strip}(<span {if $cart.stored_taxes == "Y"}class="hidden"{/if} data-ca-switch-id="manual_taxes">{include file="common/modifier.tpl" mod_value=$tax.rate_value/2 mod_type=$tax.rate_type})</span>
