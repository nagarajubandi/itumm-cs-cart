...
{capture name="sidebar"}
    ...
    <div class="sidebar-row" id="categories_stats">
        ...
        <ul class="unstyled sidebar-stat">
           ...
           {if !($runtime.company_id)} <li>{__("hidden_categories")} <span>{$categories_stats.categories_hidden}</span></li>
            <li>{__("disabled_categories")} <span>{$categories_stats.categories_disabled}</span></li>{/if}
        </ul>
