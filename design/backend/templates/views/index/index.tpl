        ...
        <tr>
            <td width="25%">
                <div class="dashboard-card">
                    ...
                    <div class="dashboard-card-content">
                        <a href="{"orders.manage?is_search=Y&payments[]=12&status=C&period=C&time_from=`$time_from`&time_to=`$time_to`"|fn_url}">
                        <h3>
                            {include file="common/price.tpl" value=$dashboard_stats.balance|default:0}
                        </h3>
                        </a>
                        &nbsp;
                    </div>
            ...
            <td width="25%">
                <div class="dashboard-card">
                    ...
                    <div class="dashboard-card-content">
                        <a href="{"companies.balance?is_search=Y&payout_type=payout&approval_status=C&period=C&time_from=`$time_from`&time_to=`$time_to`"|fn_url}">
                        <h3>{include file="common/price.tpl" value=$dashboard_stats.last_payment.payout_amount|default:0}</h3>
                        </a>
                        {if !empty($dashboard_stats.last_payment)}
                            On {$dashboard_stats.last_payment.payout_date|date_format:"`$settings.Appearance.date_format`, `$settings.Appearance.time_format`"}
                        {else}
                           &nbsp;
                        {/if}
                    </div>
            ...
            <td width="25%">
                <div class="dashboard-card">
                    ...
                    <div class="dashboard-card-content">
                        <a href="{"orders.manage?is_search=Y&status=C&period=C&time_from=`$time_from`&time_to=`$time_to`"|fn_url}">
                        <h3>{include file="common/price.tpl" value=$dashboard_stats.sales|default:0}</h3>
                        </a>
                        &nbsp;
                    </div>
                </div>
            ...
            <td width="25%">
                <div class="dashboard-card">
                    ...
                    <div class="dashboard-card-content">
                        <a href="{"orders.taxes?is_search=Y&period=C&time_from=`$time_from`&time_to=`$time_to`"|fn_url}">
                        <h3>{include file="common/price.tpl" value=$orders_stat.taxes.subtotal|default:0}</h3>
                        </a>
                        &nbsp;
            ...
            <tr>
                <td width="25%">
                    <div class="dashboard-card">
                        ...
                        <div class="dashboard-card-content">
                            <a href="{"orders.manage?is_search=Y&payments[]=12&status=C&period=C&time_from=`$time_from`&time_to=`$time_to`"|fn_url}">
                            <h3>{include file="common/price.tpl" value=$dashboard_stats.balance|default:0}</h3>
                            </a>
                            &nbsp;
                        </div>
                    </div>
                ...
                <td width="25%">
                    <div class="dashboard-card">
                        <div class="dashboard-card-title">{__("taxes")}</div>
                        <div class="dashboard-card-content">
                            <a href="{"orders.taxes?is_search=Y&period=C&time_from=`$time_from`&time_to=`$time_to`&status[]=C"|fn_url}">
                            <h3>{include file="common/price.tpl" value=$orders_stat.taxes.subtotal|default:0}</h3>
                            &nbsp;</a>
                            {*{include file="common/price.tpl" value=$orders_stat.taxes.prev_subtotal}, {if $orders_stat.taxes.subtotal > $orders_stat.taxes.prev_subtotal}+{/if}{$orders_stat.taxes.diff nofilter}%*}
                        </div>
                ...
                <td width="25%">
                    <div class="dashboard-card">
                        ...
                        <div class="dashboard-card-content">
                            <a href="{"companies.commissions?is_search=Y&period=C&time_from=`$time_from`&time_to=`$time_to`"|fn_url}">
                            <h3>{include file="common/price.tpl" value=$dashboard_stats.commissions|default:0}</h3>
                            </a>
                            &nbsp;
                        </div>
                    </div>
        ...
        {/if}
            ...
            {*<tr>
                ...
                {if !empty($orders_stat.orders)}
                    <td>
                        <div class="dashboard-card">
                            <div class="dashboard-card-title">{_("ALL ORDERS")}</div>
                            <div class="dashboard-card-content">
                                <h3>
                ...
                <td>
                    <div class="dashboard-card">
                        <div class="dashboard-card-title">Sales (EBS + COD)</div>
                        <div class="dashboard-card-content">
                            *}{*<a href="{"orders.manage?is_search=Y&period=C&status[]=C"|fn_url}">*}{*
                            <h3>{include file="common/price.tpl" value=$dashboard_stats.sales|default:0}</h3>
                            *}{*</a>*}{*
                            &nbsp;
                        </div>
        ...
        <table class="table table-middle table-last-td-align-right">
            ...
            {foreach from=$orders.0 item="order"}
                <tr>
                    ...
                    <td style="width: 35%"><a href="{"orders.details?order_id=`$order.order_id`"|fn_url}"><bdi>{__("order")}</bdi> #{$order.order_id}</a> {__("by")} {if $order.user_id}<a href="{"profiles.update?user_id=`$order.user_id`"|fn_url}">{/if}{$order.lastname} {$order.firstname}{if $order.user_id}</a>{/if}</td>
                    <td style="width: 15%"><span class="date">{$order.timestamp|date_format:"`$settings.Appearance.date_format`, `$settings.Appearance.time_format`"}</span></td>
                    <td style="width: 15%"><h4>{include file="common/price.tpl" value=$order.total}</h4></td>
                </tr>
            {foreachelse}
                <tr><td>{if $status == 'D'}
        ...
        {if !empty($order_statuses)}
             <div class="dashboard-recent-orders cm-j-tabs tabs" style="width: 98%;">
                ...
                <ul class="nav nav-pills">
					...
					{if $status.description != "All"}
					{if $status.status != "F" && $status.status != "D"}
                            ...
                            <table class="table table-striped">
                                ...
                                {foreach from=$order_by_statuses item="order_status"}
                                    ...
                                    <tr>
                                        ...
                                        <td width="25%"><a href="{$url}">{$order_status.count}</a></td>
                                        <td width="25%"><a href="{$url}">{include file="common/price.tpl" value=$order_status.total}</a></td>
                                        <td width="25%"><a href="{$url}">{include file="common/price.tpl" value=$order_status.shipping}</a></td>
                                    </tr>
                                {/foreach}
                                </tbody>
                            </table>
        ...
        <div class="welcome-screen">
            ...
            <div class="welcome-location-wrapper clearfix">
                <div class="welcome-location-block pull-left center">
                    <h4 class="install-title">{_("'iTumm Sellerhub")}</h4>
