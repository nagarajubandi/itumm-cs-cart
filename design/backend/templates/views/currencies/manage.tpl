...
{capture name="sidebar"}
    ...
    <div class="sidebar-row">
        <h6 class="hidden yahoo">{__("exchange_rate")}</h6>
        <ul class="unstyled currencies-rate" id="currencies_stock_exchange">
        </ul>
    </div>
    <a href="http://finance.yahoo.com/" class="currencies-powered hidden yahoo" target="_blank"
       title="Yahoo finance"></a>
    <script type="text/javascript">
        ...
        var exchangeRate = {
            ...
            parseExchangeRate: function(data) {
                ...
                var name = data.query.results.row.name !== undefined
                    ? Tygh.$.trim(data.query.results.row.name.split('to')[0])
                    : null;
                var rate = data.query.results.row.rate !== undefined
                    ? parseFloat(data.query.results.row.rate, 10)
                    : null;
                var container = Tygh.$('#currencies_stock_exchange');

                if(rate && name && name != this.primary_currency) {
